...
const Sentiment: React.FC<{ id: string }> = ({ id }) => {
  ...
  return <SentimentRenderer data={data!.sentiments} />
}

const SentimentRenderer: React.FC<{ data: Sentiments }> = ({ data }) => (
...
const Tweet: React.FC<TTweet> = (tweet) => (
  <div key={tweet.id} className="dark:bg-gray-800 p-4 my-4 rounded-lg shadow-2xl shadow-zinc-400 dark:shadow-sky-900 border dark:border-gray-600">
    ...
    <React.Suspense fallback={<h1>loading</h1>}>
      {tweet.sentiments ? <SentimentRenderer data={tweet.sentiments} /> : <Sentiment id={tweet.id} />}
    </React.Suspense>
  </div>
)
